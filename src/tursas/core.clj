...
(defn- process-cecp-command
  ...
  [cmd]
  (m/cond-match cmd
                #"^protover \d$" (do (set-option! :cecp-protocol-version (second (re-seq #"\S+" cmd)))
                ...
                #"^variant$" (set-option! :variant (second (re-seq #"\S+" cmd)))
                ...
                #"^usermove \w+$" (tursas-cmd "Can't print hint from a empty board!" user-move (second (re-seq #"\S+" cmd)))
                ;;"?" (cecp-move-now)
                #"^ping \d+$" (cecp-ping (second (re-seq #"\S+" cmd)))
                #"^draw$" (tursas-cmd "Can't offer draw to empty board!" cecp-draw)
                ;;#"^result \w+$" (cecp-result (second (re-seq #"\S+" cmd))) ;; XXX: incorrect
                #"^setboard" (set-game! cmd)
                ...
                #"^name \w+$" (set-option! :opponent-name (second (re-seq #"\S+" cmd)))
                ...
                #"^option \w+$" (cecp-parse-option (second (re-seq #"\S+" cmd)))
                ? (if (move-string? cmd)
                    (user-move cmd)
