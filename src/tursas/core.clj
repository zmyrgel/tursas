...
(defn- process-cecp-command
  ...
  (m/cond-match cmd
                ...
                #"^level$" (str "Error (Unsupported command): " cmd) ;; (set-option! :level (rest words))
                #"^st$" (str "Error (Unsupported command): " cmd) ;; (set-option! :time (second words))
                #"^sd \d+$" (set-option! :depth-limit (second (re-seq #"\S+" cmd)))
                #"^nps$" (str "Error (Unsupported command): " cmd) ;; (set-option! :nps (rest words))
                #"^time$" (str "Error (Unsupported command): " cmd) ;; (cecp-set-engine-clock (second words))
                #"^otim$" (str "Error (Unsupported command): " cmd) ;; (cecp-set-opponent-clock (second words))
                ...
                #"^\?$" (str "Error (Unsupported command): " cmd) ;; (cecp-move-now)
                ...
                #"^bk$" (str "Error (Unsupported command): " cmd) ;; (cecp-bk)
                #"^undo$" (undo-move!)
                #"^remove$" (undo-move! 2)
                #"^hard$" (str "Error (Unsupported command): " cmd) ;; (set-option! :ponder true)
                #"^easy$" (str "Error (Unsupported command): " cmd) ;; (set-option! :ponder false)
                #"^post$" (str "Error (Unsupported command): " cmd) ;; (set-option! :ponder-output true)
                #"^nopost" (str "Error (Unsupported command): " cmd) ;; (set-option! :ponder-output false)
                #"^analyse$" (str "Error (Unsupported command): " cmd) ;; (cecp-analyse-mode)
                #"^name \w+$" (set-option! :opponent-name (s/drop 5 cmd))
                #"^rating$" (cecp-send-rating)
                #"^ics$" (str "Error (Unsupported command): " cmd) ;; (cecp-ics)
                #"^computer$" (set-option! :opponent :cpu)
                #"^pause$" (str "Error (Unsupported command): " cmd) ;;(cecp-pause)
                #"^resume$" (str "Error (Unsupported command): " cmd) ;; (cecp-resume)
                #"^memory$" (str "Error (Unsupported command): " cmd) ;; (cecp-set-memory (second words))
                #"^cores$" (str "Error (Unsupported command): " cmd) ;;(cecp-set-cores (second words))
                #"^egtpath$" (str "Error (Unsupported command): " cmd) ;;(cecp-set-egtpath (second words))
