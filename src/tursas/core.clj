...
(defn- process-cecp-command
  ...
  (m/cond-match cmd
                ...
                #"^level \d+ [0-9:]+ \d+$" (unsupported-command cmd) ;; (set-option! :level (s/drop 6 command))
                #"^st \d+$" (unsupported-command cmd) ;; (set-option! :time (Integer/parseInt (s/drop 3 command)))
                #"^sd \d+$" (set-option! :depth-limit (s/drop 3 cmd))
                #"^nps \d+$" (unsupported-command cmd) ;; (set-option! :nps (Integer/parseInt (s/drop 4 command))
                #"^time \d+$" (unsupported-command cmd) ;; (set-option! :engine-clock (Integer/parseInt (s/drop 5 command)))
                #"^otim \d+$" (unsupported-command cmd) ;; (set-option! :opponent-clock (Integer/parseInt (s/drop 5 command)))
